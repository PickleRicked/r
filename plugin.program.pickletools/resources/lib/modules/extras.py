...
def PLAYERCORE_ANDROID():
	...
	if pass_me == 0:
		...
		dialog.ok(ADDONTITLE, "[COLOR white]playercorefactory.xml installed![/COLOR]",'',"[COLOR white]Thank you for using Pickle Tools![/COLOR]")
...
def YOUTUBE_REMOVE():
	...
	if os.path.exists(YOUTUBE_DATA):
		try:
			...
			dialog.ok(ADDONTITLE,"[COLOR white]There was an error removing the YouTube addon data folder. Thank you for using Pickle Tools[/COLOR]")
			quit()
	try:
		...
		dialog.ok(ADDONTITLE,"[COLOR white]There was an error creating the YouTube addon data settings. Thank you for using Pickle Tools[/COLOR]")
		quit()
	dialog.ok(ADDONTITLE,"[COLOR white]The YouTube plugin should now be fixed and working correctly. Thank you for using Pickle Tools[/COLOR]")
