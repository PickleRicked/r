...
ADDONPATH      = os.path.join(os.path.join(xbmc.translatePath('special://home'), 'addons'),'plugin.program.pickletools')
...
MAINTTITLE     = "[COLOR aqua]Pickle[/COLOR] [COLOR white]Maintenance Tools[/COLOR]"
EXCLUDES       = ['plugin.program.pickletools','plugin.video.picklewizard','script.module.requests','temp','kodi.log','kodi.log.old','spmc.log','spmc.log.old','dbmc.log','dbmc.log.old']
...
def deleteAddonDB():
    ...
    if codename == "Pass":
                ...
                for name in files:
                    if "addons" in name.lower():
                        try:
                            ...
                            dialog.ok(MAINTTITLE,str(name)+  "removed!",'','[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
                        except: 
                            dialog.ok(MAINTTITLE,'Error Removing ' + str(name),'','[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
                            pass
                    else:
                        continue
        except:
        ...
        dialog.ok(MAINTTITLE,'This feature is not available in Kodi 17 Krypton','','[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
...
def GET_ADDON_STATS():
    ...
    if check_build=="true":
        b = "[COLOR limegreen]Yes[/COLOR]"
    else:
        b = "[COLOR lightskyblue]No[/COLOR]"
    Common.addItem('[COLOR ghostwhite]Version: [/COLOR][COLOR limegreen]%s' % version + " " + codename + "[/COLOR]",BASEURL,200,ICON,FANART,'')
    ...
    Common.addItem('[COLOR ghostwhite]Local IP: [/COLOR][COLOR limegreen]' + s.getsockname()[0] + '[/COLOR]',BASEURL,200,ICON,FANART,'')
    Common.addItem('[COLOR ghostwhite]External IP: [/COLOR][COLOR limegreen]' + m.group(0) + '[/COLOR]',BASEURL,200,ICON,FANART,'')
...
def CHECK_BROKEN_SOURCES():
    ...
    try:
        for item in match:
            ...
            if "http" in str(checker):
                dp.update(0,"","[COLOR limegreen][B]Checking: " + name + "[/B][/COLOR]", "")
                try:
    ...
    dialog.ok(ADDONTITLE,'[COLOR white]We have checked your sources and found:[/COLOR]', '[COLOR limegreen][B]WORKING SOURCES: ' + str(passed) + ' [/B][/COLOR]','[COLOR red][B]DEAD SOURCES: ' + str(counter) + ' [/B][/COLOR]')
...
def CHECK_BROKEN_REPOS():
    ...
    dp.update(0,"[COLOR limegreen][B]We are currently checking:[/B][/COLOR]",'',"[COLOR limegreen][B]Alive: 0[/B][/COLOR][COLOR red][B]        Dead: 0[/B][/COLOR]")
    url = HOME
            ...
            if file == "addon.xml":
                ...
                if "info compressed=" in str(a):
                    ...
                    for checker in match:
                        dp.update(0,"","[COLOR limegreen][B]" + checker + "[/B][/COLOR]", "")
                        try:
                            ...
                            if error_out == 0:
                                if not "addon id=" in ("%s" % checkme.text):    
                                    ...
                                    for repo_id,repo_name in match:
                                        ...
                                        choice = xbmcgui.Dialog().yesno(ADDONTITLE,"[COLOR white]The [/COLOR][COLOR limegreen]" + repo_name + "[/COLOR] [COLOR white] appears to be broken. We attempted to connect to the repo but it was unsuccessful.[/COLOR]",'[COLOR red]To remove this repository please click YES[/COLOR]',yeslabel='[B][COLOR limegreen]YES[/COLOR][/B]',nolabel='[B][COLOR lightskyblue]NO[/COLOR][/B]')
                                        if choice == 1:
                                            try:
                        ...
                        if dp.iscanceled():
                            ...
                            dp.close()
                            sys.exit()
                        dp.update(0,"","","[COLOR limegreen][B]Alive: " + str(passed) + "[/B][/COLOR][COLOR red][B]        Dead: " + str(failed) + "[/B][/COLOR]")
                        
    dialog.ok(ADDONTITLE,'[COLOR white]We have checked your repositories and found:[/COLOR]', '[COLOR limegreen][B]WORKING SOURCES: ' + str(passed) + ' [/B][/COLOR]','[COLOR red][B]DEAD SOURCES: ' + str(failed) + ' [/B][/COLOR]')
        ...
        for root, dirs, files in os.walk(url):
            for file in files:
                if file.endswith(".xml"):
                     dp.update(0,"Fixing","[COLOR limegreen]" + file + "[/COLOR]", "Please wait.....")
                     a=open((os.path.join(root, file))).read()
...
def DeleteCrashLogs():  
    ...
    if os.path.exists(HomeDir)==True:   
        ...
        dialog = xbmcgui.Dialog()
        dialog.ok(MAINTTITLE, "Crash logs deleted", "[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]")
    else:
        dialog = xbmcgui.Dialog()
        dialog.ok(MAINTTITLE, "An error occured", "[COLOR smokewhite]Please report this to Pickle Tools[/COLOR]")
...
def HidePasswords():
    ...
    if dialog.yesno(MAINTTITLE, "Are You Sure You Want To Hide Passwords?", ""):
        ...
        dialog.ok(MAINTTITLE, "All Passowrds are now hidden!", "[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]") 
...
def UnhidePasswords():
    ...
    if dialog.yesno(MAINTTITLE, "Are You Sure You Want To Make All Passwords Visable?", ""):
        ...
        dialog.ok(MAINTTITLE, "All Passowrds are now visable!", "[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]") 
                ...
                if ".old.log" not in name.lower():
                    if ".log" in name.lower():
                        ...
                        if found == 0:
                            ...
                            Common.TextBoxError('[COLOR limegreen][B]Pickle Tools - ERRORS[/B][/COLOR]',"%s" % c)
                            sys.exit(0)
...
def view_LastError():
    ...
    if len(errors) > 0:
        ...
        Common.TextBoxError('[COLOR limegreen][B]Pickle Tools - ERRORS[/B][/COLOR]',msg)
    else: 
        dialog.ok(MAINTTITLE,'Great news! We did not find any errors in your log.')
...
def view_Errors():
    ...
    if len(errors) > 0:
        ...
        Common.TextBoxError('[COLOR limegreen][B]Pickle Tools - ERRORS[/B][/COLOR]',string.replace('NEW_L','\n').replace('NEW_R','\r'))
    else:
        dialog.ok(MAINTTITLE,'Great news! We did not find any errors in your log.')
...
def viewLogFile():
    ...
    if len(logsfound_old) > 0:
        choice = xbmcgui.Dialog().yesno(MAINTTITLE, '[COLOR white]Found: [/COLOR]' + '[COLOR limegreen][B]' + str(name_current) + ' (CURRENT)[/B][/COLOR]','[COLOR white]Found: [/COLOR]' + '[COLOR limegreen][B]' + str(name_old) + ' (OLD)[/B][/COLOR]','Which log would you like to view?',nolabel='[B]CURRENT[/B]', yeslabel='[B]OLD[/B]')
        if choice == 0:
            ...
            if file == True: return logsfound_current[-1]
                ...
                logtext     = filename.read()
                filename.close()
                Common.TextBoxError('[COLOR limegreen]Pickle Tools - Viewing ' + name_current + '[/COLOR]',logtext)
        else:
            ...
            if file == True: return logsfound_old[-1]
                ...
                logtext     = filename.read()
                filename.close()
                Common.TextBoxError('[COLOR limegreen]Pickle Tools - Viewing ' + name_old + '[/COLOR]',logtext)
    else:
        logsfound_current.sort(key=lambda f: os.path.getmtime(f))
        if file == True: return logsfound_current[-1]
        else:
...
def RUYA_FIX():
    ...
    dialog.ok(ADDONTITLE, "[COLOR white]RUYA Fix installed![/COLOR]",'',"[COLOR white]Thank you for using Pickle Tools![/COLOR]")
...
def AUTO_CLEAN_ON_OFF():
    ...
    HOME         =  xbmc.translatePath('special://userdata/addon_data/plugin.program.pickletools')
...
def convertSize(size):
   import math
   if (size == 0):
       return '[COLOR limegreen][B]0 MB[/COLOR][/B]'
   size_name = ("B", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB")
   i = int(math.floor(math.log(size,1024)))
   p = math.pow(1024,i)
   s = round(size/p,2)
   if size_name[i] == "B":
        return '[COLOR limegreen][B]%s %s' % (s,size_name[i]) + '[/COLOR][/B]'
   if size_name[i] == "KB":
        return '[COLOR limegreen][B]%s %s' % (s,size_name[i]) + '[/COLOR][/B]'
   if size_name[i] == "GB":
