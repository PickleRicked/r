...
MaintTitle="[COLOR green]Pickle[/COLOR] [COLOR white]Maintenance Tools[/COLOR]"
...
def SHOWCOMMUNITYBUILDS(name, url, description):
	...
	try:
		...
		if passed == 0:
			dialog.ok(AddonTitle, "Sorry the password entered was not found.",'[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
			sys.exit(0)
	...
	try:
		...
		Common.TextBox('[COLOR yellowgreen][B]Pickle Tools Community Builds[/B][/COLOR]',"%s" % msg)
	except: pass
...
def SHOWPROTECTEDBUILDS(name, url, description):
	...
	try:
		f = open(COM_NOTICE,mode='r'); msg = f.read(); f.close()
		Common.TextBox('[COLOR green][B]No Limits Tools Community Builds[/B][/COLOR]',"%s" % msg)
	except: pass
