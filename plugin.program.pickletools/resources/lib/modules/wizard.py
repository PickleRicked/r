...
ADDON_ID      	 = uservar.ADDON_ID  #'plugin.program.pickletools'
...
INCLUDEANARCHY   = ADDON.getSetting('includeanarchy')
INCLUDEBEAUS   	 = ADDON.getSetting('includebeausplace')
INCLUDEBENNU   	 = ADDON.getSetting('includebennu')
INCLUDEBONE   	 = ADDON.getSetting('includebonecrusher')
INCLUDEBOXSET  	 = ADDON.getSetting('includeboxsetkings')
INCLUDEBUBBLE  	 = ADDON.getSetting('includebubbles')
INCLUDECOVEN   	 = ADDON.getSetting('includecovenant')
...
INCLUDEEXOSHARK	 = ADDON.getSetting('includeexoshark')
INCLUDEFLIXAN 	 = ADDON.getSetting('includeflixanity')
INCLUDEGURZIL  	 = ADDON.getSetting('includegurzil')
INCLUDEHITLISTS	 = ADDON.getSetting('includehitlists')
INCLUDENEPTUNE	 = ADDON.getSetting('includeneptune')
INCLUDENOTSURE   = ADDON.getSetting('includenotsure')
INCLUDEPIRANHA 	 = ADDON.getSetting('includepiranha')
INCLUDEPOSEIDON	 = ADDON.getSetting('includeposeidon')
INCLUDEPROJM   	 = ADDON.getSetting('includeprojectm')
INCLUDEREDPILL 	 = ADDON.getSetting('includeredpill')
INCLUDERESIST  	 = ADDON.getSetting('includeresistance')
INCLUDEREVEL  	 = ADDON.getSetting('includerevelations')
INCLUDESALTS  	 = ADDON.getSetting('includesalts')
INCLUDESALTSHD 	 = ADDON.getSetting('includesaltshdlite')
INCLUDESALTSRD 	 = ADDON.getSetting('includesaltsrdlite')
INCLUDESILENT  	 = ADDON.getSetting('includesilenthunter')
INCLUDESPECTO  	 = ADDON.getSetting('includespecto')
INCLUDESPORTIE 	 = ADDON.getSetting('includesportie')
INCLUDESTREAMH 	 = ADDON.getSetting('includestreamhub')
INCLUDETHEDEL  	 = ADDON.getSetting('includethedelorean')
INCLUDETINKLEP 	 = ADDON.getSetting('includetinklepad')
INCLUDETRIANGU 	 = ADDON.getSetting('includetriangulum')
INCLUDEUKTURK  	 = ADDON.getSetting('includeukturk')
INCLUDEWTFORK  	 = ADDON.getSetting('includewtfork')
INCLUDEXXXODUS 	 = ADDON.getSetting('includexxxodus')
...
LOGFILES       	 = ['log', 'xbmc.old.log', 'kodi.log', 'kodi.old.log', 'spmc.log', 'spmc.old.log', 'tvmc.log', 'tvmc.old.log', 'ftmc.log', 'ftmc.old.log', 'cemc.log', 'cemc.old.log']
...
def getCacheSize():
	...
	dbfiles   = [
		(os.path.join(ADDOND, 'plugin.video.AnArchy', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.Beausplace', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.bennu', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.bonecrusher', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.boxsetkings', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.bubbles', 'cache2.db')),
		...
		(os.path.join(ADDOND, 'plugin.video.exoshark', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.flixanity', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.gurzil', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.hitlists', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.neptune', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.piranha', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.poseidon', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.projectm', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.redpill', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.resistance', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.revelations', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.sedundnes', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.silenthunter', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.specto', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.sportie', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.streamhub', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.TheDeLorean', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.tinklepad', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.triangulum', 'cache.db')),
		(os.path.join(ADDOND, 'plugin.video.xxx-o-dus', 'cache.db')),
		...
		(os.path.join(DATABASE,  'UKTurk.db')),
		(os.path.join(DATABASE,  'wtforkcache.db'))]
	cachelist = [
		(PROFILEADDONDATA),
		(ADDONDATA),
	...
	if INCLUDEVIDEO == 'true':
		...
		if INCLUDEALL == 'true': files = dbfiles
			...
			if INCLUDEANARCHY == 'true':  files.append(os.path.join(ADDOND, 'plugin.video.AnArchy', 'cache.db'))
			if INCLUDEBEAUS == 'true':    files.append(os.path.join(ADDOND, 'plugin.video.Beausplace', 'cache.db'))
			if INCLUDEBENNU == 'true':    files.append(os.path.join(ADDOND, 'plugin.video.bennu', 'cache.db'))
			if INCLUDEBONE == 'true':     files.append(os.path.join(ADDOND, 'plugin.video.bonecrusher', 'cache.db'))
			if INCLUDEBOXSET == 'true':   files.append(os.path.join(ADDOND, 'plugin.video.boxsetkings', 'cache.db'))
			if INCLUDEBUBBLE == 'true':   files.append(os.path.join(ADDOND, 'plugin.video.bubbles', 'cache2.db'))
			...
			if INCLUDEEXOSHARK == 'true': files.append(os.path.join(ADDOND, 'plugin.video.exoshark', 'cache.db'))
			if INCLUDEFLIXAN == 'true':   files.append(os.path.join(ADDOND, 'plugin.video.flixanity', 'cache.db'))
			if INCLUDEGURZIL == 'true':   files.append(os.path.join(ADDOND, 'plugin.video.gurzil', 'cache.db'))
			if INCLUDEHITLISTS == 'true': files.append(os.path.join(ADDOND, 'plugin.video.hitlists', 'cache.db'))
			...
			if INCLUDEPIRANHA == 'true':  files.append(os.path.join(ADDOND, 'plugin.video.piranha', 'cache.db'))
			if INCLUDEPOSEIDON == 'true': files.append(os.path.join(ADDOND, 'plugin.video.poseidon', 'cache.db'))
			if INCLUDEPROJM == 'true':    files.append(os.path.join(ADDOND, 'plugin.video.projectm', 'cache.db'))
			if INCLUDEREDPILL == 'true':  files.append(os.path.join(ADDOND, 'plugin.video.redpill', 'cache.db'))
			if INCLUDERESIST == 'true':   files.append(os.path.join(ADDOND, 'plugin.video.resistance', 'cache.db'))
			if INCLUDEREVEL == 'true':    files.append(os.path.join(ADDOND, 'plugin.video.revelations', 'cache.db'))
			if INCLUDESILENT == 'true':   files.append(os.path.join(ADDOND, 'plugin.video.silenthunter', 'cache.db'))
			if INCLUDESPECTO == 'true':   files.append(os.path.join(ADDOND, 'plugin.video.specto', 'cache.db'))
			if INCLUDESPORTIE == 'true':  files.append(os.path.join(ADDOND, 'plugin.video.sportie', 'cache.db'))
			if INCLUDESTREAMH == 'true':  files.append(os.path.join(ADDOND, 'plugin.video.streamhub', 'cache.db'))
			if INCLUDETHEDEL == 'true':   files.append(os.path.join(ADDOND, 'plugin.video.TheDeLorean', 'cache.db'))
			if INCLUDETINKLEP == 'true':  files.append(os.path.join(ADDOND, 'plugin.video.tinklepad', 'cache.db'))
			if INCLUDETRIANGU == 'true':  files.append(os.path.join(ADDOND, 'plugin.video.triangulum', 'cache.db'))
			if INCLUDEXXXODUS == 'true':  files.append(os.path.join(ADDOND, 'plugin.video.xxx-o-dus', 'cache.db'))
			...
			if INCLUDEWTFORK == 'true':   files.append(os.path.join(DATABASE,  'wtforkcache.db'))
		if len(files) > 0:
			for item in files: totalsize = getSize(item, totalsize)
		else: 
			log("Clear Cache: Clear Video Cache Not Enabled", xbmc.LOGNOTICE)
			#LogNotify("[COLOR %s]Clear Cache[/COLOR]" % COLOR1, '[COLOR %s]Include Video Cache Not Enabled[/COLOR]' % COLOR2)
	return totalsize
	...
	if os.path.exists(dep):
		...
		for depends in match:
			if not 'xbmc.python' in depends:
				...
				if not DP == None: 
					DP.update("","[COLOR %s]Checking Dependency [COLOR yellow]%s[/COLOR] for [COLOR yellow]%s[/COLOR]" % (COLOR2, depends, name),"")
				if os.path.exists(dependspath):
					toggleAddon(name, 'true')
			xbmc.sleep(100)

def toggleAdult():
	do = DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Would you like to [COLOR %s]Enable[/COLOR] or [COLOR %s]Disable[/COLOR] all Adult addons?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR %s]Enable[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Disable[/COLOR][/B]" % COLOR1)
	...
	if len(found) > 0: 
		choice = DIALOG.yesno("[COLOR %s]Enable/Disable Adult[/COLOR]" % COLOR1, "[COLOR %s]Would you like to view a list of the Adult addons that were %s?[/COLOR]" % (COLOR2, goto.replace('ing', 'ed')), yeslabel="[B][COLOR %s]View List[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Cancel[/COLOR][/B]" % COLOR1)
		if choice == 1: 
			editlist = '[CR]'.join(found)
			TextBox("[B][COLOR %s]%s[/COLOR][/B]" % (COLOR1, ADDONTITLE), "[COLOR %s]Here is a list of the addons that were %s for Adult Content:[/COLOR][CR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, goto.replace('ing', 'ed'), COLOR1, editlist))
		forceUpdate()
		LogNotify("[COLOR %s]Enable/Disable Adult[/COLOR]" % COLOR1, "[COLOR %s][COLOR %s]%d[/COLOR] Adult Addon(s) %s[/COLOR]" % (COLOR2, COLOR1, len(found), goto.replace('ing', 'ed')))
	else: 
		LogNotify("[COLOR %s]Enable/Disable Adult[/COLOR]" % COLOR1, "[COLOR %s]No Adult Addons Found![/COLOR]" % COLOR2)
		sys.exit()
...
def toggleAddon(id, value, over=None):
	...
	if os.path.exists(addonxml):
		...
		try:
			if len(tid) > 0:
				addonid = tid[0]
			else: LogNotify("[COLOR %s]Enable/Disable Addons[/COLOR]" % COLOR1, "[COLOR %s]No Addons Found![/COLOR]" % COLOR2)
			if len(tservice) > 0:
	...
	if 'error' in response and over == None:
		...
		DIALOG.ok("[COLOR %s]Enable/Disable Addons[/COLOR]" % COLOR1, "[COLOR %s]Error %s: [COLOR %s]%s[/COLOR]" % (COLOR2, v, COLOR1, id), "[COLOR %s]Check to make sure the addon list is up to date and try again.[/COLOR]" % COLOR2)
		forceUpdate()
...
def createQR():
	url = getKeyboard('', "[COLOR %s]%s: Insert the URL for the QRCode.[/COLOR]" % (COLOR1, ADDONTITLE))
	if url == "": LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Create QR Code: Cancelled![/COLOR]' % COLOR2); return
	if not url.startswith('http://') and not url.startswith('https://'): LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Crete QR: Not a Valid URL![/COLOR]' % COLOR2); return
	...
	if not working == True:
		if not DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]It seems the URL you entered isn't working.[/COLOR]" % COLOR2, "[COLOR %s]Would you like to create it anyway?[/COLOR]" % COLOR1, "[COLOR %s]%s[/COLOR]" % (COLOR1, working), yeslabel="[B][COLOR %s]Yes, Create[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Cancel[/COLOR][/B]" % COLOR1):
			return
	name = getKeyboard('', "[COLOR %s]Insert the name for the QRCode:[/COLOR]" % COLOR1)
	name = "QrImage_%s" % id_generator(6) if name == "" else name
	image = generateQR(url, name)
	DIALOG.ok("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]The QRCode image has been created and is located in the addondata directory:[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, image.replace(HOME, '')))
...
def cleanupBackup():
	...
	list = []; filelist = []
	if len(folder) == 0:
		LogNotify("[COLOR %s]Clean Up Backup Folder[/COLOR]" % COLOR1, "[COLOR %s]Location Empty![/COLOR]" % (COLOR2))
		return
	...
	selected = DIALOG.select("[COLOR %s]Select the items to remove from 'MyBuilds':[/COLOR]" % COLOR1, list)
	if selected == -1:
		LogNotify("[COLOR %s]Clean Up Backup Folder[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
	elif selected == 0: 
		if DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Would you like to clean up [COLOR %s]ALL[/COLOR] items in your [COLOR %s]'My_Builds'[/COLOR] folder?[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, MYBUILDS), yeslabel="[B][COLOR %s]Clean ALL[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Cancel[/COLOR][/B]" % COLOR1):
			clearedfiles, clearedfolders = cleanHouse(xbmc.translatePath(MYBUILDS))
			LogNotify("[COLOR %s]Clean Up Backup Folder[/COLOR]" % COLOR1, "[COLOR %s]Removed File(s): [COLOR %s]%s[/COLOR] / Folder(s):[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, clearedfiles, COLOR1, clearedfolders))
		else:
			LogNotify("[COLOR %s]Clean Up Backup Folder[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
	else:
		path = filelist[selected-1]; passed = False
		if DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Would you like to remove [COLOR %s]%s[/COLOR] from [COLOR %s]'My_Builds'[/COLOR] folder?[/COLOR]" % (COLOR2, COLOR1, list[selected],COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), yeslabel="[B][COLOR %s]Clean Up[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Cancel[/COLOR][/B]" % COLOR1):
			...
			if passed: LogNotify("[COLOR %s]Clean Up Backup Folder[/COLOR]" % COLOR1, "[COLOR %s]%s Removed![/COLOR]" % (COLOR2, list[selected]))
			else: LogNotify("[COLOR %s]Clean Up Backup Folder[/COLOR]" % COLOR1, "[COLOR %s]Error Removing %s![/COLOR]" % (COLOR2, list[selected]))
		else:
			LogNotify("[COLOR %s]Clean Up Backup Folder[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
...
def forceUpdate(silent=False):
	...
	if silent == False: LogNotify("[COLOR %s]Force Update Addons[/COLOR]" % COLOR1, "[COLOR %s]Please Wait...[/COLOR]" % COLOR2)

def convertSpecial(url, over=False):
	total = fileCount(url); start = 0
	DP.create("[COLOR %s]Convert Paths to Special[/COLOR]" % COLOR1, "[COLOR %s]Changing Physical Paths To Special[/COLOR]" % COLOR2, '', "[COLOR %s]Please Wait...[/COLOR]" % COLOR1)
	for root, dirs, files in os.walk(url):
		for file in files:
			start += 1
			perc = int(percentage(start, total))
			if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
				DP.update(perc, "[COLOR %s]Scanning: [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, root.replace(HOME, '')), "[COLOR %s]%s[/COLOR]" % (COLOR1, file), "[COLOR %s]Please Wait...[/COLOR]" % COLOR2)
				...
				if DP.iscanceled(): 
					DP.close()
					LogNotify("[COLOR %s]Convert Paths to Special[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
					sys.exit()
	log("[Convert Paths to Special] Completed", xbmc.LOGNOTICE)
	if over == False: LogNotify("[COLOR %s]Convert Paths to Special[/COLOR]" % COLOR1, "[COLOR %s]Completed[/COLOR]" % COLOR2)
...
def clearCrash():  
	...
	for file in glob.glob(os.path.join(LOG, '*stacktrace*.*')):
		files.append(file)
	if len(files) > 0:
		choice = DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Would you like to delete the Crash logs?' % COLOR2, '[COLOR %s]%s[/COLOR] Files Found[/COLOR]' % (COLOR1, len(files)), yeslabel="[B][COLOR %s]Remove Logs[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Keep Logs[/COLOR][/B]" % COLOR1)
		if choice == 1:
			for f in files:
				os.remove(f)
			LogNotify("[COLOR %s]Clear Crash Logs[/COLOR]" % COLOR1, '[COLOR %s]%s Log(s) Removed[/COLOR]' % (COLOR2, len(files)))
		else: 
			LogNotify("[COLOR %s]Clear Crash Logs[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
			sys.exit()
	else: 
		LogNotify("[COLOR %s]Clear Crash Logs[/COLOR]" % COLOR1, "[COLOR %s]None Found![/COLOR]" % COLOR2)
		sys.exit()

def hidePassword():
	if DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Would you like to [COLOR %s]HIDE[/COLOR] all passwords when typing in the add-on settings menus?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR %s]Hide[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Cancel[/COLOR][/B]" % COLOR1):
		count = 0
		...
		LogNotify("[COLOR %s]Hide Passwords[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
		log("[Hide Passwords] Cancelled!", xbmc.LOGNOTICE)

def unhidePassword():
	if DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Would you like to [COLOR %s]UNHIDE[/COLOR] all passwords when typing in the add-on settings menus?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR %s]Unhide[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Cancel[/COLOR][/B]" % COLOR1):
		count = 0
		for folder in glob.glob(os.path.join(ADDONS, '*/')):
			sett = os.path.join(folder, 'resources', 'settings.xml')
			if os.path.exists(sett):
		...
		LogNotify("[COLOR %s]Unhide Passwords[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
...
def wizardUpdate(startup=None):
	if workingURL(WIZARDFILE):
		...
		zip = checkWizard('zip')
		if ver > VERSION:
			yes = DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]There is a new version of the [COLOR %s]%s[/COLOR] available!' % (COLOR2, COLOR1, ADDONTITLE), 'Would you like to download [COLOR %s]v%s[/COLOR]?[/COLOR]' % (COLOR1, ver), nolabel="[B][COLOR %s]Remind Me Later[/COLOR][/B]" % COLOR1, yeslabel="[B][COLOR %s]Update Now[/COLOR][/B]" % COLOR2)
			if yes:
				log("[Auto Update Wizard] Installing wizard v%s" % ver, xbmc.LOGNOTICE)
				DP.create("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Downloading Wizard Update...[/COLOR]' % COLOR2, '', '[COLOR %s]Please Wait...[/COLOR]' % COLOR1)
				lib=os.path.join(PACKAGES, '%s-%s.zip' % (ADDON_ID, ver))
				try: os.remove(lib)
				except: pass
				downloader.download(zip, lib, DP)
				xbmc.sleep(2000)
				DP.update(0,"", "[COLOR %s]Installing %s update[/COLOR]" % (COLOR2, ADDONTITLE))
				...
				ebi('UpdateLocalAddons()')
				xbmc.sleep(1000)
				LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Update complete[/COLOR]' % COLOR2)
			...
			if not startup: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]No New Version of the [COLOR %s]%s[/COLOR] is available." % COLOR2, COLOR1, ADDONTITLE)
...
def convertText():
	...
	DP.create("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s][B]Converting Text:[/B][/COLOR]' % (COLOR2), '', '[COLOR %s]Please Wait...[/COLOR]' % COLOR1)
	
	if not BUILDFILE == 'http://':
		filename = os.path.join(TEXTFILES, 'builds.txt')
		writing = '';x = 0
		a = openURL(BUILDFILE).replace('\n','').replace('\r','').replace('\t','')
		DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]builds.txt[/COLOR]' % (COLOR2, COLOR1),'', '[COLOR %s]Please Wait...[/COLOR]' % COLOR2)
		...
		if len(match2) == 0:
			for name, version, url, gui, kodi, theme, icon, fanart in match:
				...
				if not theme == 'http://':
					...
					DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]%s_theme.txt[/COLOR]' % (COLOR2, COLOR1, name),'', '[COLOR %s]Please Wait...[/COLOR]' % COLOR2)
	...
	if not APKFILE == 'http://':
		...
		DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]apks.txt[/COLOR]' % (COLOR2, COLOR1), '', '[COLOR %s]Please Wait...[/COLOR]' % COLOR2)
	...
	if not YOUTUBEFILE == 'http://':
		...
		DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]youtube.txt[/COLOR]' % (COLOR2, COLOR1), '', '[COLOR %s]Please Wait...[/COLOR]' % COLOR2)
	...
	if not ADVANCEDFILE == 'http://':
		...
		DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]advancedsettings.txt[/COLOR]' % (COLOR2, COLOR1), '', '[COLOR %s]Please Wait...[/COLOR]' % COLOR2)
		match = re.compile('name="(.+?)".+?rl="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(a)
		for name, url, icon, fanart, description in match:
			x += 1
			DP.update(int(percentage(x, len(match))), '', "[COLOR %s]%s[/COLOR]" % (COLOR1, name))
	...
	DP.close()
	DIALOG.ok("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Your text files have been converted to 0.1.7 and are located in the [COLOR %s]/addon_data/%s/[/COLOR] folder[/COLOR]' % (COLOR2, COLOR1, ADDON_ID))
...
def asciiCheck(use=None, over=False):
	if use == None:
		source = DIALOG.browse(3, '[COLOR %s]Select the folder you wish to scan:[/COLOR]' % COLOR1, 'files', '', False, False, HOME)
		if over == True:
			yes = 1
		else:
			yes = DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Do you want to [COLOR %s]Delete ALL[/COLOR] filenames with special characters or would you rather just [COLOR %s]Scan and view[/COLOR] the results in the log?[/COLOR]' % (COLOR2, COLOR1, COLOR1), yeslabel='[B][COLOR %s]Delete[/COLOR][/B]' % COLOR2, nolabel='[B][COLOR %s]Scan[/COLOR][/B]' % COLOR1)
	else: 
		source = use
		yes = 1
	...
	DP.create("[COLOR %s]Remove Non-Ascii Filenames[/COLOR]" % COLOR1, '[COLOR %s]Please Wait...[/COLOR]' % COLOR2)
	for base, dirs, files in os.walk(source):
		...
		for file in files:
			...
			DP.update(prog2, "[COLOR %s]Checking for Non-Ascii files" % COLOR2,'[COLOR %s]%s[/COLOR]' % (COLOR1, d), '[COLOR %s]Please Wait...[/COLOR]' % COLOR2)
			try:
				if DP.iscanceled(): 
					DP.close()
					LogNotify("[COLOR %s]Remove Non-Ascii Filenames[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
					sys.exit()
				file.encode('ascii')
				...
				if yes:
					try: 
						...
						afiles.write('\n')
						f1 += 1
						log("[Non-Ascii Check] File Removed: %s " % badfile, xbmc.LOGERROR)
					except:
						for chunk in chunks(badfile, 75):
							afails.write(chunk+'\n')
						afails.write('\n')
						f2 += 1
						log("[Non-Ascii Check] File Failed: %s " % badfile, xbmc.LOGERROR)
				else:
					for chunk in chunks(badfile, 75):
						afiles.write(chunk+'\n')
					afiles.write('\n')
					f1 += 1
					log("[Non-Ascii Check] File Found: %s " % badfile, xbmc.LOGERROR)
				pass
	...
	if total > 0:
		...
		if yes:
			if use:
				LogNotify("[COLOR %s]Remove Non-Ascii Filenames[/COLOR]" % COLOR1, "[COLOR %s][COLOR %s]%s[/COLOR] Removed / [COLOR %s]%s[/COLOR] Failed[/COLOR]" % (COLOR2, COLOR1, f1, COLOR1, f2))
			else:
				TextBox("[COLOR %s]Remove Non-Ascii Filenames[/COLOR]" % COLOR1, "[B][COLOR %s]%s [COLOR %s]File(s) Removed:[/COLOR][/B]\n %s\n\n[B][COLOR %s]%s [COLOR %s]File(s) Failed:[/COLOR][/B]\n %s" % (COLOR1, f1, COLOR2, msg, COLOR2, f2, COLOR1, msg2))
		else: 
			TextBox("[COLOR %s]Remove Non-Ascii Filenames[/COLOR]" % COLOR1, "[COLOR %s][B]%s File(s) Found:[/B][/COLOR]\n %s" % (COLOR2, f1, msg))
	else: LogNotify("[COLOR %s]Remove Non-Ascii Filenames[/COLOR]" % COLOR1, "[COLOR %s]None Found![/COLOR]" % COLOR2)
...
def convertAdvanced():
	if os.path.exists(ADVANCED):
		...
		if KODIV >= 17:
			...
			return
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s][COLOR %s]advancedSettings.xml[/COLOR] NOT found![/COLOR]" %(COLOR2, COLOR1))
...
def backUpOptions(type, name=""):
	...
	exclude_files = ['Textures13.db', '.DS_Store', 'advancedsettings.xml', 'Thumbs.db', '.gitignore']
	bad_files     = [os.path.join(DATABASE, 'DEATHScache.db'), 
					 ...
					 os.path.join(DATABASE, 'UKTurk.db'),
					 os.path.join(DATABASE, 'wtforkcache.db'),
					 os.path.join(DATABASE, 'wtforkcache.db-shm'), 
					 os.path.join(DATABASE, 'wtforkcache.db-wal'),
	...
	try:
		...
		DIALOG.ok("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Error making Backup directories:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, str(e)))
		return
	if type == "build":
		if DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Are you sure you wish to back up the current Build?[/COLOR]" % COLOR2, nolabel="[B][COLOR %s]Cancel Backup[/COLOR][/B]" % COLOR1, yeslabel="[B][COLOR %s]Back Up Build[/COLOR][/B]" % COLOR2):
			if name == "":
				name = getKeyboard('', "[COLOR %s]Please enter a name for the Build zip file:[/COLOR]" % COLOR1)
				if not name:
					LogNotify("[COLOR %s]Back Up Build[/COLOR]" % COLOR1, "[COLOR %s]No File Name Entered![/COLOR]" % COLOR2)
					return False
			...
			for_progress  = 0
			ITEM          = []
			if not DIALOG.yesno("[COLOR %s]Back Up Build[/COLOR]" % COLOR1, "[COLOR %s]Do you want to include your addon_data folder?" % COLOR2, "[COLOR %s]This contains [COLOR %s]ALL[/COLOR] addon settings including passwords, but may also contain important information such as skin shortcuts. [COLOR %s]It is recommended to [COLOR %s]MANUALLY[/COLOR] remove the addon_data folders that aren't required." % (COLOR1, COLOR2, COLOR2, COLOR1), "[COLOR %s]The addon_data for [COLOR %s]%s[/COLOR] is ignored.[/COLOR]" % (COLOR1, COLOR2, ADDON_ID), yeslabel="[B][COLOR %s]Include Data[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Don't Include[/COLOR][/B]" % COLOR1):
			...
			try:
				...
				try:
					...
					log("Unable to create %s.zip file" % name, xbmc.LOGERROR)
					if DIALOG.yesno("[COLOR %s]Back Up Build[/COLOR]" % COLOR1, "[COLOR %s]We are unable to write to the current backup directory, would you like to change the location?[/COLOR]" % COLOR2, yeslabel="[B][COLOR %s]Change Directory[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Cancel[/COLOR][/B]" % COLOR1):
						openS()
						return
					else:
						LogNotify("[COLOR %s]Back Up Build[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
						return
			DP.create("[COLOR %s]Back Up Build[/COLOR]" % COLOR1, "[COLOR %s]Creating backup zip:[/COLOR]" % COLOR2, '', "[COLOR %s]Please Wait...[/COLOR]" % COLOR2)
			for base, dirs, files in os.walk(HOME):
				dirs[:] = [d for d in dirs if d not in exclude_dirs]
				files[:] = [f for f in files if f not in exclude_files]
				for file in files:
			...
			for base, dirs, files in os.walk(HOME):
				...
				for file in files:
					try:
						...
						DP.update(int(progress), '[COLOR %s]Creating backup zip: [COLOR%s]%s[/COLOR] / [COLOR%s]%s[/COLOR]' % (COLOR2, COLOR1, for_progress, COLOR1, N_ITEM), '[COLOR %s]%s[/COLOR]' % (COLOR1, file), '')
						if DP.iscanceled(): 
							DP.close()
							LogNotify("[COLOR %s]Back Up Build[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
							sys.exit()
						...
						try:
							...
							log("[Back Up] Type = '%s': Unable to back up %s" % (type, file), xbmc.LOGNOTICE)
							log("%s / %s" % (Exception, e))
					except Exception, e:
						log("[Back Up] Type = '%s': Unable to back up %s" % (type, file), xbmc.LOGNOTICE)
			...
			zipf.close()
			xbmc.sleep(500)
			DP.update(100, "[COLOR %s]Back Up Build[/COLOR]" % COLOR2, "[COLOR %s]Creating %s_guisettings.zip file[/COLOR]" % (COLOR1, name), '')
			...
			DP.close()
			DIALOG.ok("[COLOR %s]Back Up Build[/COLOR]" % COLOR1, "[COLOR %s]%s[/COLOR] [COLOR %s]Build zip successful:[/COLOR]" % (COLOR2, name, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR2, zipname))
		else: LogNotify("[COLOR %s]Back Up Build[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
	elif type == "guifix":
		if DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Are you sure you wish to back up the current GuiFix?[/COLOR]" % COLOR2, nolabel="[B][COLOR %s]Cancel Backup[/COLOR][/B]" % COLOR1, yeslabel="[B][COLOR %s]Back Up GuiFix[/COLOR][/B]" % COLOR2):
			if name == "":
				guiname = getKeyboard('', "[COLOR %s]Please enter a name for the GuiFix zip file:[/COLOR]" % COLOR1)
				if not guiname:
					LogNotify("[COLOR %s]Back Up GuiFix[/COLOR]" % COLOR1, "[COLOR %s]No File Name Entered![/COLOR]" % COLOR2)
					return False
					#sys.exit()
			...
			if os.path.exists(GUISETTINGS):
				try:
					...
					try:
						...
						log("Unable to create %s_guisettings.zip file" % guiname, xbmc.LOGERROR)
						if DIALOG.yesno("[COLOR %s]Back Up GuiFix[/COLOR]" % COLOR1, "[COLOR %s]We are unable to write to the current backup directory, would you like to change the location?[/COLOR]" % COLOR2, yeslabel="[B][COLOR %s]Change Directory[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Cancel[/COLOR][/B]" % COLOR1):
							openS()
							return
						else:
							return
				try:
					...
					for fold in match:
						...
						if not fd in ['skin.confluence', 'skin.re-touch', 'skin.estuary', 'skin.estouchy']:
							if DIALOG.yesno("[COLOR %s]Back Up GuiFix[/COLOR]" % COLOR1, "[COLOR %s]Would you like to add the following skin folder to the GuiFix Zip file?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, fd), yeslabel="[B][COLOR %s]Add Skin[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Skip Skin[/COLOR][/B]" % COLOR1):
			...
			if name == "":
				DIALOG.ok("[COLOR %s]Back Up GuiFix[/COLOR]" % COLOR1, "[COLOR %s]GuiFix zip successful:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, guizipname))
		else: LogNotify("[COLOR %s]Back Up GuiFix[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
	elif type == "theme":
		if not DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Are you sure you wish to back up the current Theme?[/COLOR]" % COLOR2, yeslabel="[B][COLOR %s]Continue[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Cancel[/COLOR][/B]" % COLOR1): LogNotify("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2); return False
		if name == "":
			themename = getKeyboard('', "[COLOR %s]Please enter a name for the Theme zip file:[/COLOR]" % COLOR1)
			if not themename: 
				LogNotify("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]No File Name Entered![/COLOR]" % COLOR2)
				return False
		else: themename = name
		...
		try:
			...
			try:
				...
				log("Unable to create %s.zip file" % themename, xbmc.LOGERROR)
				if DIALOG.yesno("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]We are unable to write to the current backup directory, would you like to change the location?[/COLOR]" % COLOR2, yeslabel="[B][COLOR %s]Change Directory[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Cancel[/COLOR][/B]" % COLOR1):
					openS()
					return
				else:
					return
		...
		try:
			if not SKIN == 'skin.confluence':
				...
				if len(match2) > 1:
					if DIALOG.yesno("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]Would you like to go through the Texture Files for " % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLOR %s]Add Textures[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Skip Textures[/COLOR][/B]" % COLOR1):
						skinfold = os.path.join(ADDONS, SKIN, 'media')
						match2 = glob.glob(os.path.join(skinfold,'*.xbt'))
						for xbt in match2:
							if DIALOG.yesno("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]Would you like to add the Texture File [COLOR %s]%s[/COLOR]" % (COLOR1, COLOR2, xbt.replace(skinfold, "")[1:]), "from [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLOR %s]Add Textures[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Skip Textures[/COLOR][/B]" % COLOR1):
								fn  = xbt
								fn2 = fn.replace(HOME, "")
								zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
				else:
					for xbt in match2:
						if DIALOG.yesno("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]Would you like to add the Texture File [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, xbt.replace(skinfold, "")[1:]), "from [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLOR %s]Add Textures[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Skip Textures[/COLOR][/B]" % COLOR1):
							fn  = xbt
							fn2 = fn.replace(HOME, "")
							zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
				ad_skin = os.path.join(ADDOND, SKIN, 'settings.xml')
				if os.path.exists(ad_skin):
					if DIALOG.yesno("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]Would you like to add the [COLOR %s]settings.xml[/COLOR] file in [COLOR %s]/addon_data/[/COLOR]" % (COLOR2, COLOR1, COLOR1), "for [COLOR %s]%s[/COLOR]?[/COLOR]"  % (COLOR1, SKIN), yeslabel="[B][COLOR %s]Add Settings[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Skip Settings[/COLOR][/B]" % COLOR1):
				...
				if 'script.skinshortcuts' in match:
					if DIALOG.yesno("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]Would you like to add the [COLOR %s]settings.xml[/COLOR] file for [COLOR %s]script.skinshortcuts[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR %s]Add Settings[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Skip Settings[/COLOR][/B]" % COLOR1):
						for base, dirs, files in os.walk(os.path.join(ADDOND,'script.skinshortcuts')):
							files[:] = [f for f in files if f not in exclude_files]
							for file in files:
								fn = os.path.join(base, file)
								zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
			if DIALOG.yesno("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]Would you like to include a [COLOR %s]Backgrounds[/COLOR] folder?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR %s]Yes, Include[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Continue[/COLOR][/B]" % COLOR1):
				fn = DIALOG.browse(0, '{COLOR %s]Select location of backgrounds:[/COLOR]' % COLOR1, 'files', '', True, False, HOME, False)
				if not fn == HOME:
				...
				if DIALOG.yesno("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]Would you like to include the [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, text), yeslabel="[B][COLOR %s]Yes, Include[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Continue[/COLOR][/B]" % COLOR1):
					zipf.write(os.path.join(DATABASE, text), '/userdata/Database/%s' % text, zipfile.ZIP_DEFLATED)
			if DIALOG.yesno("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]Would you like to include any addons?[/COLOR]" % (COLOR2), yeslabel="[B][COLOR %s]Yes, Include[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Continue[/COLOR][/B]" % COLOR1):
				...
				for folder in sorted(fold, key = lambda x: x):
					...
					if os.path.exists(xml):
						...
						if len(match) > 0:
							...
							addonfolds.append(foldername)
				if KODIV > 16:
					selected = DIALOG.multiselect("[COLOR %s]Select the addons you wish to add to the zip file:[/COLOR]" % COLOR1, addonnames)
				else:
					selected = []; choice = 0
					tempaddonnames = ["-- Click here to Continue --"] + addonnames
					while not choice == -1:
						choice = DIALOG.select("[COLOR %s]Select the addons you wish to add to the zip file:[/COLOR]" % COLOR1, tempaddonnames)
						if choice == -1: break
						elif choice == 0: break
						else: 
							choice2 = (choice-1)
					...
					for item in selected:
						for base, dirs, files in os.walk(os.path.join(ADDONS,addonfolds[item])):
							...
							for file in files:
								...
								fn = os.path.join(base, file)
								zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
			if DIALOG.yesno("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]Would you like to include the [COLOR %s]guisettings.xml[/COLOR] file?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR %s]Yes, Include[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Continue[/COLOR][/B]" % COLOR1):
				zipf.write(GUISETTINGS, '/userdata/guisettings.xml', zipfile.ZIP_DEFLATED)
		except Exception, e:
			zipf.close()
			log("[Back Up] Type = '%s': %s" % (type, str(e)), xbmc.LOGNOTICE)
			DIALOG.ok("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]%s[/COLOR][COLOR %s] backup failed:[/COLOR]" % (COLOR1, themename, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, str(e)))
			if not tempzipname == '':
		...
		if not tempzipname == '':
			...
			if success == 0:
				xbmcvfs.copy(tempzipname, zipname)
				xbmcvfs.delete(tempzipname)
		DIALOG.ok("[COLOR %s]Back Up Theme[/COLOR]" % COLOR1, "[COLOR %s]%s[/COLOR][COLOR %s] Theme zip successful:[/COLOR]" % (COLOR2, themename, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR2, zipname))
	elif type == "addondata":
		if DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Are you sure you wish to back up the current Addon_data?[/COLOR]" % COLOR2, nolabel="[B][COLOR %s]Cancel Backup[/COLOR][/B]" % COLOR1, yeslabel="[B][COLOR %s]Back Up Data[/COLOR][/B]" % COLOR2):
			if name == "":
				name = getKeyboard('', "[COLOR %s]Please enter a name for the Addon_data zip file:[/COLOR]" % COLOR1)
				if not name:
					LogNotify("[COLOR %s]Back Up Addon_data[/COLOR]" % COLOR1, "[COLOR %s]No File Name Entered![/COLOR]" % COLOR2)
					return False
			...
			try:
				...
				try:
					...
					if DIALOG.yesno("[COLOR %s]Back Up Addon_data[/COLOR]" % COLOR1, "[COLOR %s]We are unable to write to the current backup directory, would you like to change the location?[/COLOR]" % COLOR2, yeslabel="[B][COLOR %s]Change Directory[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Cancel[/COLOR][/B]" % COLOR1):
						openS()
						return
					else:
						return
			...
			DP.create("[COLOR %s]Back Up Addon_data[/COLOR]" % COLOR1, "[COLOR %s]Creating backup zip[/COLOR]" % COLOR2, '', "[COLOR %s]Please Wait...[/COLOR]" % COLOR1)
			...
			for base, dirs, files in os.walk(ADDOND):
				...
				if DP.iscanceled(): 
					DP.close()
					LogNotify("[COLOR %s]Back Up Addon_data[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
					sys.exit()
				for file in files:
					try:
						for_progress += 1
			...
			if not tempzipname == '':
				...
				if success == 0:
					...
					xbmcvfs.delete(tempzipname)
			DP.close()
			DIALOG.ok("[COLOR %s]Back Up Addon_data[/COLOR]" % COLOR1, "[COLOR %s]%s[/COLOR] [COLOR %s]Addon_data zip successful:[/COLOR]" % (COLOR2, name, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR2, zipname))
		else: LogNotify("[COLOR %s]Back Up Addon_data[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2) 
...
def restoreLocal(type):
	...
	try:
		...
		DIALOG.ok("[COLOR %s]Restore Local %s[/COLOR]" % (COLOR1, type), "[COLOR %s]Error making Backup directories:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, str(e)))
		return
		sys.exit()
	file = DIALOG.browse(1, '[COLOR %s]Select the [COLOR %s]Local %s[/COLOR] backup zip file you wish to restore:[/COLOR]' % (COLOR1, COLOR2, type), 'files', '.zip', False, False, mybuilds)
	log("[Restore Local %s] File: %s " % (type.upper(), file), xbmc.LOGNOTICE)
	if file == "" or not file.endswith('.zip'):
		LogNotify("[COLOR %s]Restore Local %s[/COLOR]" % (COLOR1, type), "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
		return
		sys.exit()
	DP.create("[COLOR %s]Restore Local %s[/COLOR]" % (COLOR1, type), "[COLOR %s]Installing Local Backup[/COLOR]" % COLOR2, '', "[COLOR %s]Please Wait...[/COLOR]" % COLOR1)
	...
	try:
		...
		DP.update(0, '[COLOR %s]Unable to read zipfile from current location.' % COLOR2, '[COLOR %s]Copying file to packages.[/COLOR]' % COLOR1)
		pack = os.path.join('special://home', 'addons', 'packages', fn)
		xbmcvfs.copy(file, pack)
		file = xbmc.translatePath(pack)
		DP.update(0, '', '[COLOR %s]Copying file to packages: Complete[/COLOR]' % COLOR1)
		zipfile.ZipFile(file, 'r')
	percent, errors, error = extract.all(file,loc,DP)
	if DP.iscanceled(): 
		DP.close()
		LogNotify("[COLOR %s]Restore Local %s[/COLOR]" % (COLOR1, type), "[COLOR %s]Install Cancelled![/COLOR]" % COLOR2)
		sys.exit()
	fixmetas()
	clearS('build')
	DP.close()
	...
	if int(errors) >= 1:
		yes=DIALOG.yesno("[COLOR %s]Restore Local %s[/COLOR]" % (COLOR1, type), '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, fn), 'Completed: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Would you like to view the errors?[/COLOR]', nolabel="[B][COLOR %s]No, Thanks[/COLOR][/B]" % COLOR1, yeslabel="[B][COLOR %s]View Errors[/COLOR][/B]" % COLOR2)
		if yes:
			if isinstance(errors, unicode):
				error = error.encode('utf-8')
			TextBox("[COLOR %s]Local Restore[/COLOR]" % COLOR1, error.replace('\t',''))
	setS('installed', 'true')
	setS('extract', str(percent))
	setS('errors', str(errors))
	if INSTALLMETHOD == 1: todo = 1
	elif INSTALLMETHOD == 2: todo = 0
	else: todo = DIALOG.yesno("[COLOR %s]Restore Local %s[/COLOR]" % (COLOR1, type), "[COLOR %s]Would you like to [COLOR %s]Force Close[/COLOR] Kodi or [COLOR %s]Reload Profile[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR %s]Reload Profile[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Force Close[/COLOR][/B]" % COLOR1)
	if todo == 1: reloadFix()
	else: killxbmc(True)

def restoreExternal(type):
	source = DIALOG.browse(1, '[COLOR %s]Select the [COLOR %s]External %s[/COLOR] backup zip file you wish to restore:[/COLOR]' % (COLOR1, type), 'files', '.zip', False, False)
	if source == "" or not source.endswith('.zip'):
		LogNotify("[COLOR %s]Restore External %s[/COLOR]" % (COLOR1, type), "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
		return
		sys.exit()
	if not source.startswith('http'):
		LogNotify("[COLOR %s]Restore External %s[/COLOR]" % (COLOR1, type), "[COLOR %s]Invalid URL![/COLOR]" % COLOR2)
		return
		sys.exit()
	try: 
		work = workingURL(source)
	except:
		LogNotify("[COLOR %s]Restore External %s[/COLOR]" % (COLOR1, type), "[COLOR %s]Error with Valid URL![/COLOR]" % COLOR2)
		log("Not a working URL, if source was local then use Restore Local option", xbmc.LOGNOTICE)
		log("External Source: %s" % source, xbmc.LOGNOTICE)
		return
		sys.exit()
	log("[Restore External %s] File: %s " % (type.upper(), source), xbmc.LOGNOTICE)
	zipit = os.path.split(source); zname = zipit[1]
	DP.create("[COLOR %s]Restore External %s[/COLOR]" % (COLOR1, type), '[COLOR %s]Downloading Zip file[/COLOR]' % COLOR2, '', '[COLOR %s]Please Wait...[/COLOR]' % COLOR1)
	if type == "gui": loc = USERDATA
	...
	DP.update(0,'[COLOR %s]Installing External %s Backup[/COLOR]' % (COLOR2, type), '', '[COLOR %s]Please Wait...[/COLOR]' % COLOR2)
	percent, errors, error = extract.all(file,loc,DP)
	if DP.iscanceled(): 
		DP.close()
		LogNotify("[COLOR %s]Restore External %s[/COLOR]" % (COLOR1, type), "[COLOR %s]Install Cancelled![/COLOR]" % COLOR2)
		sys.exit()
	fixmetas()
	clearS('build')
	DP.close()
	...
	lookandFeelData('save')
	if int(errors) >= 1:
		yes=DIALOG.yesno("[COLOR %s]Restore External %s[/COLOR]" % (COLOR1, type), '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, zname), 'Completed: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Would you like to view the errors?[/COLOR]', nolabel='[B][COLOR %s]No, Thanks[/COLOR][/B]' % COLOR1, yeslabel='[B][COLOR %s]View Errors[/COLOR][/B]' % COLOR2)
		if yes:
			TextBox("[COLOR %s]Restore External %s[/COLOR]" % (COLOR1, type), error.replace('\t',''))
	setS('installed', 'true')
	setS('extract', str(percent))
	setS('errors', str(errors))
	try: os.remove(file)
	...
	else: todo = DIALOG.yesno("[COLOR %s]Restore External %s[/COLOR]" % (COLOR1, type), "[COLOR %s]Would you like to [COLOR %s]Force Close[/COLOR] Kodi or [COLOR %s]Reload Profile[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR %s]Reload Profile[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Force Close[/COLOR][/B]" % COLOR1)
	if todo == 1: reloadFix()
	else: killxbmc(True)
	...
	if   do == 'edit':
		...
		for item in currentWhite:
			...
			if id in addonids:
				...
				tempaddonnames[pos] = "[B][COLOR %s]%s[/COLOR][/B]" % (COLOR2, name)
			else:
				addonids.append(id)
		...
		while not choice in [-1, 0]:
			choice = DIALOG.select("[COLOR %s]Select the addons you wish to add to your WhiteList:[/COLOR]" % COLOR1, tempaddonnames)
			if choice == -1: break
			elif choice == 0: break
			else: 
				choice2 = (choice-1)
		...
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]%s Addon(s) in your WhiteList[/COLOR]" % (COLOR2, len(selected)))
	elif do == 'read' :
		white = []
		...
		if os.path.exists(WHITELIST):
			list = whiteList('read')
			if len(list) > 0:
				msg = "[COLOR cyan]WhiteList items (along with dependencies) will NOT be removed when performing a Fresh Start nor when the userdata is overwritten during a build install.[CR][COLOR magenta]Here is a list of your current WhiteList items:[CR][CR][/COLOR]"
				for item in list:
					try: name, id, fold = item
					except Exception, e: log(str(e))
					msg += "[COLOR %s]%s[/COLOR] [COLOR %s]\"%s\"[/COLOR][CR]" % (COLOR1, name, COLOR2, id) 
				TextBox("[B][COLOR %s]%s[/COLOR][/B]" % (COLOR1, ADDONTITLE), msg)
			else:
				LogNotify("[COLOR %s]View My WhiteList[/COLOR]" % COLOR1, "[COLOR %s]No items in WhiteList![/COLOR]" % COLOR2)
		else:
			LogNotify("[COLOR %s]View My WhiteList[/COLOR]" % COLOR1, "[COLOR %s]No WhiteList exists![/COLOR]" % (COLOR2))
	elif do == 'import':
		source = DIALOG.browse(1, '[COLOR %s]Select the [COLOR %s]whitelist.txt[/COLOR] file you wish to import:[/COLOR]' % (COLOR1, COLOR2), 'files', '.txt', False, False, HOME)
		log(str(source))
		if not source.endswith('.txt'):
			LogNotify("[COLOR %s]Import My WhiteList[/COLOR]" % COLOR1, "[COLOR %s]No [COLOR %s]whitelist.txt[/COLOR] File Selected![/COLOR]" % (COLOR2, COLOR1))
			return
		...
		with open(WHITELIST, 'a') as f:
			for item in lines:
				...
				if not id in idList:
					...
					f.write(writing)
			LogNotify("[COLOR %s]Import My WhiteList[/COLOR]" % COLOR1, "[COLOR %s]%s Item(s) Added[/COLOR]" % (COLOR2, count))
	elif do == 'export':
		if os.path.exists(WHITELIST):
			source = DIALOG.browse(3, '[COLOR %s]Select where you wish to export your [COLOR %s]whitelist.txt[/COLOR] file to:[/COLOR]' % (COLOR1, COLOR2), 'files', '', False, False, HOME)
			log(str(source), xbmc.LOGDEBUG)
			try:
				xbmcvfs.copy(WHITELIST, os.path.join(source, 'whitelist.txt'))
				DIALOG.ok("[COLOR %s]Export My WhiteList[/COLOR]" % COLOR1, "[COLOR %s]WhiteList has been exported to:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, os.path.join(source, 'whitelist.txt')))
				...
				if not DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]The location you selected isn't writable, would you like to select another one?[/COLOR]" % COLOR2, yeslabel="[B][COLOR %s]Change Location[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Cancel[/COLOR][/B]" % COLOR1):
					LogNotify("[COLOR %s]Export My WhiteList[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % (COLOR2, e))
				else:
					whiteList(export)
		else:
				LogNotify("[COLOR %s]Export My WhiteList[/COLOR]" % COLOR1, "[COLOR %s]No WhiteList exists![/COLOR]" % (COLOR2))
	elif do == 'clear':
		if os.path.exists(WHITELIST):
			if not DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Are you sure you want to clear your current WhiteList?" % COLOR2, "[COLOR %s]This process can't be undone![/COLOR]" % COLOR1, yeslabel="[B][COLOR %s]Yes, Remove[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]No, Cancel[/COLOR][/B]" % COLOR1):
				LogNotify("[COLOR %s]Clear My WhiteList[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % (COLOR2))
				return
			try: 
				os.remove(WHITELIST)
				LogNotify("[COLOR %s]Clear My WhiteList[/COLOR]" % COLOR1, "[COLOR %s]Cleared![/COLOR]" % (COLOR2))
			except: 
				LogNotify("[COLOR %s]Clear My WhiteList[/COLOR]" % COLOR1, "[COLOR %s]No WhiteList exists![/COLOR]" % (COLOR2))
		else:
			LogNotify("[COLOR %s]Clear My WhiteList[/COLOR]" % COLOR1, "[COLOR %s]No WhiteList exists![/COLOR]" % (COLOR2))
		...
		try:
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]%s [COLOR %s]file(s) found / [COLOR %s]%s [COLOR %s]in size.[/COLOR]" % (COLOR2, str(file_count), COLOR1, COLOR2, size, COLOR1), "[COLOR %s]Do you want to delete them?[/COLOR]" % COLOR2, nolabel="[B][COLOR %s]Don't Clear[/COLOR][/B]" % COLOR1, yeslabel="[B][COLOR %s]Clear Packages[/COLOR][/B]" % COLOR2)
					if yes:
						for f in files: os.unlink(os.path.join(root, f))
						for d in dirs: shutil.rmtree(os.path.join(root, d))
						LogNotify("[COLOR %s]Clear Packages[/COLOR]" % COLOR1,'[COLOR %s]%s file(s) Removed![/COLOR]' % (COLOR2, str(file_count)))
					else: 
						LogNotify("[COLOR %s]Clear Packages[/COLOR]" % COLOR1,'[COLOR %s]Cancelled![/COLOR]' % COLOR2)
						if not over == 'total': sys.exit()
				else: 
					LogNotify("[COLOR %s]Clear Packages[/COLOR]" % COLOR1,'[COLOR %s]None Found![/COLOR]' % COLOR2)
					if not over == 'total': sys.exit()
		except Exception, e:
			log("Clear Packages Error: %s" % str(e), xbmc.LOGERROR)
			LogNotify("[COLOR %s]Clear Packages[/COLOR]" % COLOR1,'[COLOR %s]Error![/COLOR]' % COLOR2)
			if not over == 'total': sys.exit()
	else: 
		LogNotify("[COLOR %s]Clear Packages[/COLOR]" % COLOR1, '[COLOR %s]None Found![/COLOR]' % COLOR2)
		if not over == 'total': sys.exit()
...
def clearPackagesStartup():
	...
	if os.path.exists(PACKAGES):
		...
		try:
			...
			if file_count > 0: LogNotify("[COLOR %s]Clear Packages[/COLOR]" % COLOR1, '[COLOR %s]Success: [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, convertSize(cleanupsize)))
			else: LogNotify("[COLOR %s]Clear Packages[/COLOR]" % COLOR1, '[COLOR %s]None Found![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]Clear Packages[/COLOR]" % COLOR1, '[COLOR %s]Error![/COLOR]' % COLOR2)
			log("Clear Packages Error: %s" % str(e), xbmc.LOGERROR)
	else: LogNotify("[COLOR %s]Clear Packages[/COLOR]" % COLOR1, '[COLOR %s]None Found![/COLOR]' % COLOR2)
...
def clearCache(over=None):
	...
	if INCLUDEVIDEO == 'true' and over == None:
		...
		if len(files) > 0:
			...
			log("Clear Cache: Include Video Cache Not Enabled", xbmc.LOGNOTICE)
			#LogNotify("[COLOR %s]Clear Cache[/COLOR]" % COLOR1, '[COLOR %s]Include Video Cache Not Enabled[/COLOR]' % COLOR2)
	LogNotify("[COLOR %s]Clear Cache[/COLOR]" % COLOR1, '[COLOR %s]Removed/Purged %s File(s)[/COLOR]' % (COLOR2, delfiles))

def checkSources():
	if not os.path.exists(SOURCES):
		LogNotify("[COLOR %s]Scan for Broken Sources[/COLOR]" % COLOR1, "[COLOR %s][COLOR %s]sources.xml[/COLOR] File NOT Found![/COLOR]" % (COLOR2, COLOR1))
		return False
	x      = 0
	bad    = []
	remove = []
	...
	if len(match) > 0:
		...
		DP.create("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Scanning Sources for Broken links...[/COLOR]" % COLOR2)
		for name, path, sharing in match2:
			x     += 1
			if DP.iscanceled(): break
			perc   = int(percentage(x, len(match2)))
			DP.update(perc, '', "[COLOR %s]Checking:[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, name), "[COLOR %s]%s[/COLOR]" % (COLOR1, path))
			if 'http' in path:
		...
		if DP.iscanceled(): 
			DP.close()
			LogNotify("[COLOR %s]Scan for Broken Sources[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
			sys.exit()
		if len(bad) > 0:
			choice = DIALOG.yesno("[COLOR %s]Scan for Broken Sources[/COLOR]" % COLOR1, "[COLOR %s]%s[/COLOR][COLOR %s] Source(s) have been found to be Broken!" % (COLOR1, len(bad), COLOR2),"[COLOR %s]Would you like to [COLOR %s]Remove ALL[/COLOR] or choose one by one?[/COLOR]" % (COLOR1, COLOR2), yeslabel="[B][COLOR %s]Remove ALL[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Choose to Delete[/COLOR][/B]" % COLOR1)
			if choice == 1:
				remove = bad
			else:
				for name, path, sharing, working in bad: 
					log("%s sources: %s, %s" % (name, path, working), xbmc.LOGNOTICE)
					if DIALOG.yesno("[COLOR %s]Scan for Broken Sources[/COLOR]" % COLOR1, "[COLOR %s]The [COLOR %s]%s[/COLOR] source was reported as not working!" % (COLOR2, COLOR1, name), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), "[COLOR %s]%s[/COLOR]" % (COLOR2, working), yeslabel="[B][COLOR %s]Remove Source[/COLOR][/B]" % COLOR2, nolabel="[B][COLOR %s]Keep Source[/COLOR][/B]" % COLOR1):
			...
			if len(remove) > 0:
				...
				f.write(str(a))
				f.close()
				alive = len(match2) - len(bad)
				kept = len(bad) - len(remove)
				broken = len(bad)
				removed = len(remove)
				DIALOG.ok("[COLOR %s]Scan for Broken Sources[/COLOR]" % COLOR1, "[COLOR %s]Checking sources for broken paths has been completed." % COLOR2, "[COLOR %s]Working: [COLOR %s]%s[/COLOR] | Broken: [COLOR %s]%s[/COLOR] | Kept: [COLOR %s]%s[/COLOR] | Removed: [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR1, COLOR2, alive, COLOR2, broken, COLOR2, kept, COLOR2, removed), "[CR][COLOR %s]This will take effect after the next startup![/COLOR]" % COLOR2)
				sys.exit()
			else: 
				log("No Bad Sources selected to be removed.", xbmc.LOGNOTICE)
				LogNotify("[COLOR %s]Scan for Broken Sources[/COLOR]" % COLOR1, "[COLOR %s]No Sources were Removed![/COLOR]" % COLOR2)
				sys.exit()
		else: 
			LogNotify("[COLOR %s]Scan for Broken Sources[/COLOR]" % COLOR1, "[COLOR %s]All Sources are Working[/COLOR]" % COLOR2)
			sys.exit()
	else: 
		log("No Sources Found!", xbmc.LOGNOTICE)
		LogNotify("[COLOR %s]Scan for Broken Sources[/COLOR]" % COLOR1, "[COLOR %s]No Sources Found![/COLOR]" % COLOR2)
		sys.exit()

def checkRepos():
	DP.create("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Checking Repositories...[/COLOR]' % COLOR2)
	badrepos = []
	ebi('UpdateAddonRepos')
	repolist = glob.glob(os.path.join(ADDONS,'repo*'))
	if len(repolist) == 0:
		DP.close()
		LogNotify("[COLOR %s]Scan for Broken Repositories[/COLOR]" % COLOR1, "[COLOR %s]No Repositories Found![/COLOR]" % COLOR2)
		sys.exit()
	...
	if DP.iscanceled(): 
		DP.close()
		LogNotify("[COLOR %s]Scan for Broken Repositories[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
		sys.exit()
	...
	if len(badrepos) > 0:
		msg  = "[COLOR %s]Below is a list of Repositories that did not resolve.[CR][COLOR %s]This does not mean that they are Deprecated, because sometimes hosts go down for a short period of time.[CR][COLOR %s]Please do serveral scans of your repository list before actually removing a repository just to make sure it is broken.[/COLOR][CR][CR][COLOR %s]" % (COLOR2, COLOR1, COLOR2, COLOR1)
		msg += '[CR]'.join(badrepos)
		msg += '[/COLOR]'
		TextBox("[COLOR %s]%s: Bad Repositories[/COLOR]" % (COLOR1, ADDONTITLE), msg)
	else: 
		LogNotify("[COLOR %s]Scan for Broken Repositories[/COLOR]" % COLOR1, "[COLOR %s]All Repositories are Working[/COLOR]" % COLOR2)
		sys.exit()
#############################
####KILL XBMC ###############
#####THANKS BRACKETS ########
...
def killxbmc(over=None):
	if over: choice = 1
	else: choice = DIALOG.yesno("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]You are about to close Kodi!' % COLOR2, "[COLOR %s]Would you like to continue?[/COLOR]" % COLOR1, nolabel='[B][COLOR %s] No, Cancel[/COLOR][/B]' % COLOR1, yeslabel='[B][COLOR %s]Force Close[/COLOR][/B]' % COLOR2)
	if choice == 1:
		log("Force Closing Kodi: Platform[%s]" % str(platform()), xbmc.LOGNOTICE)
		os._exit(1)
	else: LogNotify("[COLOR %s]Force Close[/COLOR]" % COLOR1, "[COLOR %s]Cancelled![/COLOR]" % COLOR2)
...
def reloadFix(default=None):
	DIALOG.ok("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]WARNING: [COLOR %s]Sometimes Reloading the Profile causes Kodi to crash![/COLOR]" % (COLOR2, COLOR1), "[COLOR %s]While Kodi is Reloading the Profile, Please Do [COLOR %s]NOT[/COLOR] Press Any Buttons![/COLOR]" % (COLOR2, COLOR1))
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	if default == None:
		lookandFeelData('save')
	redoThumbs()
...
def kodi17Fix():
	if KODIV < 17:
		LogNotify("[COLOR %s]Kodi 17 Fix[/COLOR]" % COLOR1, "[COLOR %s]This is ONLY needed for Kodi 17![/COLOR]" % COLOR2)
	else:
				...
				try:
					...
					try:
						...
						try:
							...
							if len(aid) == 0: log("Unable to enable: %s(Cannot Determine Addon ID)" % fold, xbmc.LOGERROR)
							else: log("Unable to enable: %s" % folder, xbmc.LOGERROR)
		if len(disabledAddons) > 0:
			x = 0
			DP.create("[COLOR %s]Kodi 17 Fix[/COLOR]" % COLOR1, '[COLOR %s]Enabling Disabled Addons' % COLOR2, '', '[COLOR %s]Please Wait...[/COLOR]' % COLOR2)
			for item in disabledAddons:
				...
				if DP.iscanceled(): 
					DP.close()
					LogNotify("[COLOR %s]Kodi 17 Fix[/COLOR]" % COLOR1, "[COLOR %s]Enabling Disabled Addons: [COLOR %s]Cancelled[/COLOR]![/COLOR]" % (COLOR2, COLOR1))
					sys.exit()
			forceUpdate()
			ebi("ReloadSkin()")
		else:
			LogNotify("[COLOR %s]Kodi 17 Fix[/COLOR]" % COLOR1, "[COLOR %s]No Disabled Addons Found![/COLOR]" % COLOR2)
...
def addonDatabase(addon=None, state=1):
	...
	try:
		...
		log("Error enabling addon: %s" % addon)
...
def purgeDb(name):
	...
	LogNotify("[COLOR %s]Purge Databases[/COLOR]" % COLOR1, "[COLOR %s]%s Complete[/COLOR]" % (COLOR2, show[len(show)-1]))
...
def oldThumbs():
	...
	if len(images) > 0: LogNotify("[COLOR %s]Clear Old Thumbnails[/COLOR]" % COLOR1, '[COLOR %s]%s File(s) / %s MB[/COLOR]!' % (COLOR2, str(len(images)), removed))
	else: LogNotify("[COLOR %s]Clear Old Thumbnails[/COLOR]" % COLOR1, '[COLOR %s]None Found![/COLOR]' % COLOR2)
