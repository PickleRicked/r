...
NOLIMITS_VERSION    =  os.path.join(USERDATA,'pickle_build.txt')
...
def INSTALL(name,url,description):
	...
	send_to_count = name + "|SPLIT|Pickle"
...
def INSTALL_ADVANCED(name,url,description):
	...
	dialog.ok(ADDONTITLE, "[COLOR white]Advanced Settings installed![/COLOR]","[COLOR white]You should now see an imporvment in buffering[/COLOR]","[COLOR white]Thank you for using Pickle Tools![/COLOR]")
...
def INSTALL_KEYMAP(name,url,description):
	...
	xbmc.executebuiltin("Container.Refresh")
	dialog.ok(ADDONTITLE, "[COLOR white]Custom Keymap settings installed![/COLOR]","[COLOR white]Thank you for using Pickle Tools![/COLOR]")

def INSTALLAPK(name,url,description):
	if "NULL" in url:
		dialog = xbmcgui.Dialog()
		dialog.ok(ADDONTITLE, "[B][COLOR smokewhite]Not a valid selection, please try again.[/COLOR][/B]",'[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
		sys.exit(1)
...
def INSTALLLIB(name,url,description):
	...
	if "Android" in name:
		if not xbmc.getCondVisibility('system.platform.android'):
			dialog = xbmcgui.Dialog()
			dialog.ok(ADDONTITLE + " - Android", "[B][COLOR smokewhite]Sorry, this file is only for Android devices[/COLOR][/B]",'[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
			sys.exit(1)
		else:	
			name = "librtmp.so"
	...
	if "Windows" in name:
		if not xbmc.getCondVisibility('system.platform.windows'):
			dialog = xbmcgui.Dialog()
			dialog.ok(ADDONTITLE + " - Windows", "[B][COLOR smokewhite]Sorry, this file is only for Windows devices[/COLOR][/B]",'[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
			sys.exit(1)
		else:	
			name = "librtmp.dll"
	...
	if "Linux" in name:
		if not xbmc.getCondVisibility('system.platform.linux'):
			dialog = xbmcgui.Dialog()
			dialog.ok(ADDONTITLE + " - Linux", "[B][COLOR smokewhite]Sorry, this file is only for Linux devices[/COLOR][/B]",'[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
			sys.exit(1)
		else:	
			name = "librtmp.so.1"
			path = xbmc.translatePath(os.path.join('special://home',''))
	...
	if "osx" in name:
		if not xbmc.getCondVisibility('system.platform.osx'):
			dialog = xbmcgui.Dialog()
			dialog.ok(ADDONTITLE + " - MacOSX", "[B][COLOR smokewhite]Sorry, this file is only for MacOSX devices[/COLOR][/B]",'[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
			sys.exit(1)
		else:	
	...
	if "ATV" in name:
		if not xbmc.getCondVisibility('system.platform.atv2'):
			dialog = xbmcgui.Dialog()
			dialog.ok(ADDONTITLE + " - ATV", "[B][COLOR smokewhite]Sorry, this file is only for ATV devices[/COLOR][/B]",'[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
			sys.exit(1)
		else:	
			name = "librtmp.1.dylib"
			path = xbmc.translatePath(os.path.join('special://home',''))
	...
	if "iOS" in name:
		if not xbmc.getCondVisibility('system.platform.ios'):
			dialog = xbmcgui.Dialog()
			dialog.ok(ADDONTITLE + " - iOS", "[B][COLOR smokewhite]Sorry, this file is only for iOS devices[/COLOR][/B]",'[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
			sys.exit(1)
		else:	
			name = "librtmp.1.dylib"
			path = xbmc.translatePath(os.path.join('special://home',''))
	...
	if "RPi" in name:
		if not xbmc.getCondVisibility('system.platform.rpi'):
			dialog = xbmcgui.Dialog()
			dialog.ok(ADDONTITLE + " - RPi", "[B][COLOR smokewhite]Sorry, this file is only for RPi devices[/COLOR][/B]",'[COLOR smokewhite]Thank you for using Pickle Tools[/COLOR]')
			sys.exit(1)
		else:	
			name = "librtmp.1.so"
			path = xbmc.translatePath(os.path.join('special://home',''))
...
def unzip(_in, _out, dp):
	try:
		...
		if "bytes" in str(error_traceback).lower():
			dialog.ok(ADDONTITLE, 'Sorry, your connection to the download was lost before the file could be downloaded. Please try again.','If problems persist please contact @kodiPickle or if you are downloading a Community Build please contact them with this.')
			dp.close()
			quit()
		elif "file is not a zip file" in str(error_traceback).lower():
			dialog.ok(ADDONTITLE, 'Sorry, the file is not a zip file.','If problems persist please contact @kodiPickle or if you are downloading a Community Build please contact them with this.')
			dp.close()
			quit()
		else:
			dialog.ok(ADDONTITLE, 'Sorry, there was a problem extracting the file.','If problems persist please contact @kodiPickle or if you are downloading a Community Build please contact them with this.')
			dp.close()
			quit()
