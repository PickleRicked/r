...
ADVANCEDFANART = uservar.ADVANCEDFANART

if BACKGROUND == '': BACKGROUND = FANART
elif not wiz.workingURL(BACKGROUND): BACKGROUND = FANART
#BACKGROUND     = xbmc.translatePath('special://home/addons/plugin.program.pickletools/resources/art/black.jpg')
...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "[B]You have not installed a Build directly from the [COLOR %s]%s[/COLOR] yet.\n\nSelect '[COLOR %s]Build Menu[/COLOR]' to install a Build now or '[COLOR %s]Ignore[/COLOR]' to do it later.\n\nThank you for choosing the [COLOR %s]%s[/COLOR]![/B]" % (COLOR1, ADDONTITLE, COLOR1, COLOR1, COLOR1, ADDONTITLE)
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msgcurrent = "Running latest version of installed build:\n[COLOR %s]%s[/COLOR]\n\nCurrent Version: v[COLOR %s]%s[/COLOR]\nLatest Version: v[COLOR %s]%s[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new)
		...
		def onClick(self, controlId):
			...
			elif (controlId == self.normal): self.doStandardInstall()
			else: self.doIgnore()
